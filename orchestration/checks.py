"""Asset checks wrapping :func:`local.validation.validate.validate_table` (ADR-016).
...
(:data:`local.validation.schema_registry.VALIDATION_RULES`) the CLI runs at the
end of :func:`local.pipeline.run_pipeline`. Checks surface as pass/fail badges on
...
from local.validation.validate import ValidationResult, validate_table
...
def _render_rules_table(result: ValidationResult) -> str:
    ...
    One row per :class:`local.validation.validate.CheckOutcome` — passing rules
...
def _make_check(table_name: str):
    ...
    set stays in lockstep with :data:`local.transforms.registry.SILVER_TABLES`.
